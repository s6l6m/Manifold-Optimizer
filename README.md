...
Each notebook imports `CONFIG` from [notebooks/config.py](notebooks/config.py). Override parameters by passing keyword arguments:
...
The path is resolved via `CONFIG.DATA_DIR_PATH`, so if your file has a different name please update the `pd.read_csv(...)` call in [notebooks/ba_insertion.ipynb](notebooks/ba_insertion.ipynb).
